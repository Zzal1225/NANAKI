...
export default function SectionSettings({ tab, compact = false }: SectionSettingsProps) {
  ...
  const sections = tab ? ALL_SECTIONS.filter((s) => s.tab === tab) : ALL_SECTIONS
  ...
  return (
    <>
      ...
      <Modal
        ...
        title={tab ? '섹션 설정' : '앱 구성'}
      >
        <p className="mb-4 text-xs text-text-muted">
          레고처럼 원하는 탭·섹션만 켜서 사용하세요.
        </p>
          ...
          <div className="mt-6 border-t border-border pt-4">
            ...
            {!confirmWipe ? (
              <button
                ...
                className="mt-3 w-full rounded-xl border border-danger/40 px-4 py-3 text-sm text-danger hover:bg-danger/10"
              >
                데이터 초기화
              </button>
            ) : (
              <div className="mt-3 flex flex-col gap-3 rounded-xl border border-danger/40 bg-danger/5 p-4">
                <p className="text-sm text-text-primary">
...
function ToggleRow({
  ...
  return (
    ...
    >
      <span>{label}</span>
      <span className={`h-5 w-9 rounded-full p-0.5 transition-colors ${enabled ? 'bg-accent' : 'bg-border'}`}>
        <span
